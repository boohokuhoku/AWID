...
def process_input(input_text):
    ...
    for line in lines:
        ...
        if len(columns) >= 2:
            # Apply word replacements to artwork name
...
st.write("Enter two tab-separated columns: Artwork Name and AW IDs (one pair per line). AW IDs can include non-numeric tokens (e.g., Disabled), but only numeric IDs are processed. Words like 'iphone', 'ipad', and 'airpods' will be replaced with 'phone', 'tablet', and 'earbuds' respectively in Artwork Name and Short URL.")
input_text = st.text_area("Artwork Names and AW IDs:", 
                         placeholder="e.g., I just can't sit still iPhone Case\t35221837,35226788,Disabled\nAnother iPad Artwork Name\t35207351,Disabled")
